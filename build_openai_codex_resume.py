...
def add_real_bullet_numbering(doc: Document, *, marker_at_twips: int = 260, text_at_twips: int = 540) -> int:
    ...
    p_properties.append(indentation)
...
def attach_numbering(paragraph, num_id: int) -> None:
    ...
    num_properties = p_properties.find(qn("w:numPr"))
...
def build(output_path: Path) -> None:
    ...
    properties.title = "Adam Pangelinan - Full Stack Software Engineer, Codex"
    properties.subject = "Role-specific resume for OpenAI Codex"
    properties.keywords = "TypeScript, full-stack, agents, developer tools, reliability"
    properties.author = ""
    ...
    subtitle = doc.add_paragraph(
        "FULL-STACK SOFTWARE ENGINEER | AGENT PRODUCTS, DEVELOPER TOOLS, RELIABILITY",
        style="Subtitle",
    )
    ...
    summary_run = summary.add_run(
        "Full-stack TypeScript engineer with 2 to 3 years of experience shipping agent-driven "
        "product surfaces end to end. Builds inspectable agent workflows, evaluation systems, "
        "developer-tool integrations, and human-review boundaries from ambiguous idea to tested software."
    )
    ...
    add_project_bullet(
        ...
        "reconciler, task receipts, and Playwright and Vitest coverage for agent-operated workflows.",
    ...
    add_labeled_paragraph(doc, "Languages: ", "TypeScript, JavaScript, Python, Go", after=0.6)
    add_labeled_paragraph(
        doc,
        "Product engineering: ",
        "React, Next.js, Node.js, REST APIs, Zod, live ATS data",
        after=0.6,
    )
    add_labeled_paragraph(
        doc,
        "Agent and quality: ",
        "structured outputs, streamed state, evaluations, tool use, human review, browser automation/CDP, Playwright, Vitest, pytest",
        after=0.8,
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build Adam's OpenAI Codex role-specific resume.")
