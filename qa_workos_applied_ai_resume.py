...
EXPECTED_TEXT = [
    ...
    "141 tests pass with 2 symlink skips",
...
def inspect_docx(path: Path) -> dict:
    ...
    relationship_targets = set(re.findall(r'Target="([^"]+)"[^>]*TargetMode="External"', rels_xml))
    result = {
        ...
        "missing_url_targets": sorted(EXPECTED_URLS - relationship_targets),
    ...
    result["passed"] = all(
        [
            ...
            "Applied AI Engineer" in result["title"],
            "WorkOS Applied AI" in result["subject"],
            result["hyperlink_relationships"] >= 6,
            result["hyperlink_elements"] == 7,
            not result["missing_url_targets"],
...
def inspect_pdf(path: Path) -> dict:
    ...
    result = {
        ...
        "missing_url_targets": sorted(EXPECTED_URLS - annotation_urls),
    }
    result["passed"] = all(
        [
            result["pages"] == 1,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Structural QA for Adam's WorkOS Applied AI resume."
    )
