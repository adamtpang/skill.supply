...
export function FleetFooter() {
  return (
    <footer className="mx-auto w-full max-w-3xl px-5 pb-10 sm:px-8">
      <div className="flex flex-wrap items-center gap-x-3 gap-y-1 border-t border-border pt-5 font-mono text-xs tracking-[0.08em] text-muted-foreground">
        <span>
          Part of the{" "}
          <a
            href={FLEET_HUB.url}
            className="inline-flex min-h-11 min-w-11 items-center justify-center rounded underline-offset-2 outline-none hover:text-foreground hover:underline focus-visible:ring-2 focus-visible:ring-ring/50 sm:min-h-6 sm:min-w-6"
          >
        ...
        {FLEET_SISTERS.map((s) => (
          <span key={s.url} className="flex items-center gap-3">
            <span className="hidden sm:inline" aria-hidden>&middot;</span>
            <a
              href={s.url}
              title={s.one}
              className="inline-flex min-h-11 min-w-11 items-center justify-center rounded underline-offset-2 outline-none hover:text-foreground hover:underline focus-visible:ring-2 focus-visible:ring-ring/50 sm:min-h-6 sm:min-w-6"
