...
def build(output_path: Path) -> None:
    ...
    properties.title = "Adam Pangelinan - Software Engineer, Plugin Developer Platform"
    properties.subject = "Role-specific resume for OpenAI Plugin Developer Platform"
    properties.keywords = (
        "developer tools, plugins, MCP, TypeScript, Python, full-stack, agents, APIs"
    )
    properties.author = ""
    ...
    add_labeled_paragraph(
        doc,
        "Developer platform: ",
        "REST APIs, MCP tool design, CDP, connector integrations, packaging, documentation",
        after=0.6,
    )
    add_labeled_paragraph(
        doc,
        "Product and quality: ",
        "React, Next.js, Node.js, Zod, structured outputs, evaluations, human review, Playwright, Vitest, pytest",
        after=0.8,
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build Adam's OpenAI Plugin Developer Platform role-specific resume."
    )
