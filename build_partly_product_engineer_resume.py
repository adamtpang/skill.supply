...
def build(output_path: Path) -> None:
    ...
    properties.title = "Adam Pangelinan - Product Engineer"
    properties.subject = "Role-specific resume for Partly Product Engineer, US"
    properties.keywords = (
        "product engineering, TypeScript, React, Next.js, supervised AI workflows, "
    ...
    add_labeled_paragraph(
        doc,
        "Product practice: ",
        "customer discovery, workflow mapping, rapid prototyping, human-in-the-loop design, end-to-end delivery",
        after=0.6,
    )
    add_labeled_paragraph(
        doc,
        "Agent quality: ",
        "structured outputs, streamed state, evaluations, human approval, Playwright, Vitest, pytest",
        after=0.8,
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build Adam's Partly Product Engineer resume.")
