...
STYLES = {
    "name": paragraph_style(
        "Name", font="Helvetica-Bold", size=22, leading=23, color=INK, after=0.8
    ),
    "headline": paragraph_style(
        "Headline", font="Helvetica-Bold", size=9.8, leading=10.8, color=BLUE, after=1.6
    ),
    ...
    "section": paragraph_style(
        "Section", font="Helvetica-Bold", size=10.2, leading=11, color=BLUE, before=5, after=1.8
    ),
...
def build(path: Path) -> None:
    ...
    document = SimpleDocTemplate(
        ...
        title="Adam Pangelinan - Full Stack Software Engineer, Codex",
        author="",
        subject="Role-specific resume for OpenAI Codex",
        creator="",
        producer="",
        pageCompression=1,
    )
    ...
    story.append(
        Paragraph(
            "<b>Product engineering:</b> React, Next.js, Node.js, REST APIs, Zod, live ATS data",
            STYLES["compact"],
        )
    )
    story.append(
        Paragraph(
            "<b>Agent and quality:</b> structured outputs, streamed state, evaluations, tool use, "
            "human review, browser automation/CDP, Playwright, Vitest, pytest",
            STYLES["compact"],
        )
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build Adam's OpenAI Codex resume as a PDF.")
